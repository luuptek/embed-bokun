<?php

// Exit if accessed directly.
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

$bokun = new Bokun_WP();

if ( ! $bokun->has_settings_ok() ) {
	?>
    <p>
		<?php _e( 'Please setup Bokun details in settings => bokun settings to enable automatic data import for this post.', $bokun->text_domain ); ?>
    </p>
	<?php
} else {
	?>
    <p><?php _e( 'Enter bokun ID of the product (this is found in Bokun.io).', $this->text_domain ); ?></p>
	<?php wp_nonce_field( 'verify_bokun_wp_nonce', 'bokun_wp_meta_box_nonce' ); ?>
    <label class="l" for="bokun_wp_bokun_id">
		<?php _e( 'Bokun ID', $this->text_domain ) ?>
    </label>
    <input type="text" name="_bokun_wp_bokun_id" id="bokun_wp_bokun_id"
           value="<?php echo get_post_meta( get_the_ID(), '_bokun_wp_bokun_id', true ); ?>">
    <p>
		<?php echo sprintf( __( 'Automatic data import is scheduled next on %s', TEXT_DOMAIN ), $bokun->get_datetime_with_timezone_offset() ) ?>
    </p>
	<?php
}